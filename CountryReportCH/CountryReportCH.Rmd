...
```{r}
IndustryCH <- data.frame(Industry=c("Primary_Sector", "Secondary-Sector", "Tertiary_Sector"), NumberOfCompanies=c(53010, 90483, 446760)) ## Office fédéral statistique Suisse, data 2017
IndustryCH$Industry <- as.character(IndustryCH$Industry)
```

```{r}
IndustryGraph <- ggplot(IndustryCH, aes(x="", y=NumberOfCompanies, fill=Industry))+
geom_bar(width = 1, stat = "identity")
```

```{r}
library(scales)
IndustryPie <- IndustryGraph + coord_polar("y", start=0) #+
  #geom_text(aes(y = NumberOfCompanies/3 + c(0, cumsum(NumberOfCompanies)[-length(NumberOfCompanies)]), 
            #label = percent(NumberOfCompanies/100)), size=5)
IndustryPie ## peut être mettre direct en % dans le data.frame
```
