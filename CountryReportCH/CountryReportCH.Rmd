...
author: "Valentin Henriot / SKEMA BUSINESS SCHOOL"
date: "08/03/2020"
output:
  html_document: 
    toc: yes
...
I chose Switzerland for my country report because it is a country in the centre of Europe, not part of the European Union and yet has one of the most prosperous economies in the world.
On the basis of which elements Switzerland is doing well economically, I will therefore study the development of its economy but also of its social components over the last few years. For example, it will be interesting to make a comparison with Switzerland's neighbours, particularly Germany, Italy and France, to understand how and why Switzerland is a model economy.

* First of all, I will study the data on the Swiss economy to try to understand how it works and, above all, to understand on what basis Switzerland derives such a high degree of stability.

* In a second step, my analysis will focus on social indicators in Switzerland, in particular to see whether it is possible to make a link between good economic health in Switzerland and good social health.

* In a third step, my analysis will focus on business indicators in order to understand how companies in Switzerland operate, their sector of activity and their performance. Part of this analysis will also be devoted to the analysis of indicators concerning the creation of companies in Switzerland, in order to determine whether it may be appropriate to consider opening a company in this country.
...
```{r leaflet, message=FALSE, warning=FALSE, fig.align="center", include=FALSE}
...
* __Analysis of the GDP growth in %__ (Source: WDI)
* __Analysis of the DGP per capita__ (Source: WDI)
* __Consumer price index__ (Source: BIS)
* __Switzerland's trade__ (Source: WDI)
* __Balance of payments__ (Source: AMECO)
* __Index of Economic freedom__ (Source: Heritage Foundation)
...
This graph shows us the percentage change in GDP for *Switzerland* over the period from 1971 to 2018. As we can see, the change in GDP is sometimes negative, notably in **1975 due to the first oil crisis**. Nevertheless, *Switzerland* sees the variation of its GDP most of the time positive. In fact, when the **average** over the period is calculated, the result is **1.67**, and the **median** is **1.82**. 
Moreover, **since 2009**, the variation of Swiss GDP **has not had a negative variation**.
...
IndexEcoFree <- data.frame(
  ...
  Category=c("Property Rights", "Judical Effectiveness", "Government Integrity", "Tax Burden", "Govt Spending", "Fiscal Health", "Business Freedom", "Labor Freedom", "Monetary Freedom", "Trade Freedom", "Investment Freedom", "Financial Freedom"),
  Score=c(85.3,	82.0,	88.0,	70.5,	64.8,	96.3,	75.4,	72.5,	85.2,	87.4,	85,	90)) ## 2019 Index of Economic Freedom, Heritage Foundation
```
...
IndexEcoFree1 <- ggplot(IndexEcoFree, 
  ...
  coord_flip() +
  labs(caption = "Source: SKEMA Quantum Studio (Index of Economic Freedom, Heritage Foundation)")
  ggthemes::theme_stata() + theme(legend.position = "none") + 
  theme(plot.title = element_text(hjust = 0.5))
```

```{r fig.align='center'}
IndexEcoFree1 + theme(legend.position='none', axis.text.y = element_text(face="bold", color="black", size = 8), axis.text.x = element_text(color="black", face = "bold"), plot.title = element_text(face="bold", size = 14))
```
...
* __Unemployment__ (Source: WDI)
* __Expenditure on education__ (Source: WDI)
* __Foreign population__ (Source: OECD)
* __Intentional homicide ratio__ (Source: Eurostat)
* __Risk of poverty or social exclusion__ (Source: Eurostat)
* __Social progress Index__ (Source: spiR)
...
* __Corruption Perceptions Index__ (Source: Eurostat)
* __Swiss industries__ (Source: Office fédéral statistique Suisse)
* __Industries that export the most by sector__ (Source: opendata.swiss)
* __Business Partners Export__ (Source: Administration fédérale des douanes Suisse)
* __Business Partners Import__ (Source: Administration fédérale des douanes Suisse)
* __Swiss industries with the most employees__ (Source: Handelszeitung & Bisnode 2017)
* __Swiss companies with the highest turnover__ (Source: Handelszeitung & Bisnode 2017)
* __Accessibility to setting up a business__ (Source: WDI)
* __Labor tax and contributions__ (Source: World Bank)
...
ggplot(CorruptionIndex2019) +
 aes(x = Country, colour = Country, weight = value, fill = Country) +
 geom_bar() +
 scale_color_hue() +
...
```{r include=FALSE}
IndustryCH <- data.frame(Industry=c("Primary_Sector", "Secondary_Sector", "Tertiary_Sector"), NumberOfCompanies=c(8.98, 15.33, 75.69)) ## Office fédéral statistique Suisse, data 2018
...
In 2018, there were **53010 companies** in *Switzerland* in the **primary sector**, i.e. the sector consisting of the exploitation of natural resources (e.g. agriculture, fisheries, forestry, etc.). This sector accounts for only **9%** of Swiss companies.
The **secondary sector** represents **90483 companies**, i.e. **15%** of the total number of companies. This is the sector that deals with the transformation of raw materials from the primary sector.
Finally, the **tertiary sector** is the sector representing the largest number of companies in *Switzerland*. Indeed, this sector accounts for **75%** of all Swiss companies, i.e. **446760 service companies**.
This sector alone accounts for **73% of Switzerland's GDP**.
...
```{r include=FALSE}
BusinessPartnersExport <- data.frame(
  stringsAsFactors = FALSE,
                Country = c("Allemagne", "USA", "France", "Italie", "Chine"),
                Value = c(18.8, 16.3, 6.5, 6.2, 5.2) ## in % of total exports
) ## Source : Administration fédérale des douanes Suisse
```

```{r}
BusinessPartnersExport$Country <- with(BusinessPartnersExport, reorder(Country, -Value))
```

```{r include=FALSE}
BusinessPartnersExport1 <- ggplot(BusinessPartnersExport, 
            aes(x = Country, y = Value, color = Country, fill = Country, label = Value)) +
  geom_col() +
  geom_text(nudge_y = 1) + 
  ggtitle("Where does Switzerland export the most?") + 
  xlab("None") + ylab("In % of total exports") + 
  labs(caption = "Source: SKEMA Quantum Studio (Administration fédérale des douanes Suisse)")
  ggthemes::theme_stata() + theme(legend.position = "none") + 
  theme(plot.title = element_text(hjust = 0.5))
```

```{r fig.align='center'}
BusinessPartnersExport1 + theme(legend.position='none', axis.title.x = element_blank(), axis.text.y = element_text(face="bold", color="black", size = 8), axis.text.x = element_text(color="black", face = "bold"), plot.title = element_text(face="bold", size = 14))
```

*Switzerland*'s preferred trading partner is *Germany*. The majority of *Switzerland*'s exports go to *Germany*, closely followed by the *United States*. These two partners alone account for **35.1%** of *Switzerland*'s exports.
Both countries are economically stable, however, such **dependence can have negative consequences** if the *German* or *US* **economy slows down**.

## Business Partners Import

```{r include=FALSE}
BusinessPartnersImport <- data.frame(
  stringsAsFactors = FALSE,
                Country = c("Allemagne", "USA", "Chine", "France", "Inde"),
                Value = c(15.5, 13.2, 9.7, 6.5, 5.7) ## in % of total imports
) ## Source : Administration fédérale des douanes Suisse
```

```{r include=FALSE}
BusinessPartnersImport$Country <- with(BusinessPartnersImport, reorder(Country, -Value))
```

```{r include=FALSE}
BusinessPartnersImport1 <- ggplot(BusinessPartnersImport, 
            aes(x = Country, y = Value, color = Country, fill = Country, label = Value)) +
  geom_col() +
  geom_text(nudge_y = 1) + 
  ggtitle("Where does Switzerland import the most from?") + 
  xlab("None") + ylab("In % of total imports") + 
  labs(caption = "Source: SKEMA Quantum Studio (Administration fédérale des douanes Suisse)")
  ggthemes::theme_stata() + theme(legend.position = "none") + 
  theme(plot.title = element_text(hjust = 0.5))
```
...
```{r fig.align='center'}
BusinessPartnersImport1 + theme(legend.position='none', axis.title.x = element_blank(), axis.text.y = element_text(face="bold", color="black", size = 8), axis.text.x = element_text(color="black", face = "bold"), plot.title = element_text(face="bold", size = 14))
```

Once again, *Switzerland*'s two most important import partners are *Germany* and the *USA*. Nevertheless, compared to the previous graph, *China* is now in third place for imports, whereas it was in fifth place for exports from *Switzerland*. This is partly due to the **price of labour** in *China*, which is very competitive. Conversely, the fact that the majority of products exported from *Switzerland* to *China* are **high value-added products** and that the Chinese do not have the **same purchasing power as in Europe** (or the USA) explains the "low" rate of exports from *Switzerland* to *China*.
Another interesting fact in comparison with the previous graph is the disappearance of *Italy* in this ranking of the top five import partners, which gives way to *India*.

## Swiss industries with the most employees

```{r include=FALSE}
SwissCompaniesEmployees <- data.frame(
  stringsAsFactors = FALSE,
                Company = c("Nestlé",
                       "Glencore International","ABB","Novartis","SGS","Roche","LafargeHolcim",
                       "Kühne + Nagel International","Migros-Konzern",
                       "Coop-Gruppe"),
                NumberOfEmployees = c(323000, 156000, 134800, 
                       126457, 95000, 93734, 81960, 75876, 75302, 74532) ## Handelszeitung & Bisnode 2017
)
```

```{r include=FALSE}
SwissCompaniesEmployees$Company <- with(SwissCompaniesEmployees, reorder(Company, -NumberOfEmployees))
```

```{r include=FALSE}
SwissCompaniesEmployees1 <- ggplot(SwissCompaniesEmployees, 
            aes(x = Company, y = NumberOfEmployees, color = NumberOfEmployees, label = NumberOfEmployees, fill = Company)) +
  geom_col() +
  ggtitle("Swiss industries with the most employees") + 
  xlab("Company") + ylab("Number of employees") + 
  coord_flip() +
  labs(caption = "Source: SKEMA Quantum Studio (Handelszeitung & Bisnode 2017)")
  ggthemes::theme_stata() + theme(legend.position = "none") + 
  theme(plot.title = element_text(hjust = 0.5))
```

```{r fig.align='center'}
SwissCompaniesEmployees2 <- SwissCompaniesEmployees1 + theme(legend.position='none', axis.title.y = element_blank(), axis.text.y = element_text(face="bold", color="black", size = 8), axis.text.x = element_text(color="black", face = "bold"), plot.title = element_text(face="bold", size = 14))

SwissCompaniesEmployees2 <- ggplotly(SwissCompaniesEmployees2)
SwissCompaniesEmployees2
```

The Swiss company with the most employees is **Nestlé**. Nestlé employs **323,000 people worldwide**. It is followed by **Glencore International**, which employs **156,000 people**, less than half as many as Nestlé.
Nestlé is a food company known by all, on the other hand, Glencore International is a trading, brokerage and extraction company of raw materials whose name is less well known.
**Two pharmaceutical companies** appear in this ranking: Novartis and Roche.

## Swiss companies with the highest turnover

```{r include=FALSE}
SwissCompaniesTurnover <- data.frame(
  stringsAsFactors = FALSE,
                Company = c("Glencore","Vitol",
                       "Trafigura","Cargill International","Mercuria Energy Trading",
                       "Nestlé","Gunvor","Roche","Novartis","ABB"),
                Turnover = c(202311.7,178212.6,136421,
                       108009.6,102398,89791,62029.8,53299,48352.7,33783.6) ## Handelszeitung & Bisnode 2017
)
```

```{r include=FALSE}
SwissCompaniesTurnover$Company <- with(SwissCompaniesTurnover, reorder(Company, -Turnover))
```

```{r include=FALSE}
SwissCompaniesTurnover1 <- ggplot(SwissCompaniesTurnover, 
            aes(x = Company, y = Turnover, color = Turnover, label = Turnover, fill = Company)) +
  geom_col() +
  ggtitle("Swiss industries with the highest turnover") + 
  xlab("None") + ylab("Turnover in million CHF") + 
  coord_flip() +
  labs(caption = "Source: SKEMA Quantum Studio (Handelszeitung & Bisnode 2017)")
  ggthemes::theme_stata() + theme(legend.position = "none") + 
  theme(plot.title = element_text(hjust = 0.5))
...
```{r fig.align='center'}
SwissCompaniesTurnover2 <- SwissCompaniesTurnover1 + theme(legend.position='none', axis.title.y = element_blank(), axis.text.y = element_text(face="bold", color="black", size = 8), axis.text.x = element_text(color="black", face = "bold"), plot.title = element_text(face="bold", size = 14))

SwissCompaniesTurnover2 <- ggplotly(SwissCompaniesTurnover2)
SwissCompaniesTurnover2
```

The top five companies in this ranking are **raw materials trading companies**. 
Around **25% of the world's commodity trading transactions are carried out in Switzerland** . This explains the position of these companies in this ranking in terms of turnover. For example, Trafigura, an oil brokerage company, has a turnover of **CHF 136 billion** and only **4,316 employees**!
However, in this ranking we find the company Nestlé, which is in 6th position. The pharmaceutical companies Novartis and Roche are respectively in 9th and 8th position in this ranking in terms of turnover.

## Accessibility to setting up a business

```{r include=FALSE}
BusinessCost <- WDI(indicator = "IC.REG.COST.PC.ZS", ## Cost of business start-up procedures (% of GNI per capita)
country = c("CH", "DE", "FRA", "IT"),
                   start = 2005,
                   end = 2018)
```
...
```{r fig.align='center'}
ggplot(BusinessCost) +
 aes(x = year, y = IC.REG.COST.PC.ZS, colour = country) +
 geom_line(size = 1L) +
 scale_color_hue() +
 labs(y = "Cost in % of GNI per capita", title = "Cost of business procedures (% of GNI per capita)", caption = "Source: SKEMA Quantum Studio (WDI)") +
 ggthemes::theme_gdocs()
```

This graphic explains the capital requirement for setting up a company. As we can see, this **cost is zero** for *France* and *Italy*. This means that **no contribution is required to set up a company** in these two countries (however, some types of companies still require a contribution).
As far as *Germany* is concerned, it is the country that **requires the highest contribution** (in % of the GNI per capita) to set up a company. 
*Switzerland* is in second position and to create a company in this country, it is necessary to have a **capital contribution representing on average 25% of the GNI per capita**. 
Access to setting up a company is therefore more restricted in *Germany* and *Switzerland* than in *France* or *Italy*. However, the following graph will highlight the taxes on these companies once they have been set up in order to compare the costs charged to Swiss, French, German and Italian companies. 

## Labor tax and contributions

```{r include=FALSE}
LaborTax <- rdb(ids = c('WB/DB/PAY.TAX.LABR.TAX.CONTR.ZS-CH', "WB/DB/PAY.TAX.LABR.TAX.CONTR.ZS-FR", "WB/DB/PAY.TAX.LABR.TAX.CONTR.ZS-IT", "WB/DB/PAY.TAX.LABR.TAX.CONTR.ZS-DE")) %>%
  filter(!is.na(value)) ## Paying taxes: Labor tax and contributions (% of profits) WB
```
...
ggplot(LaborTax) +
 ...
 geom_line(size = 1L) +
 scale_color_hue() +
 labs(y = "Value", title = "Labor tax and contributions (% of profits)", caption = "Source: SKEMA Quantum Studio (World Bank)") +
 ggthemes::theme_gdocs()
```

Although the cost of setting up a company is equal to 0 in *France* and *Italy*, the expenses retained (as a % of the profits made) are the **most important once the company is set up**.
On the other hand, for *Germany* and *Switzerland*, the costs are high to create the company but the **expenses charged once the company is created are lower**.
For *Switzerland*, it is the country with the **lowest expenses** and represents on average **17.7% of the profit generated** by the company.
By way of comparison, expenses in *France* represent **50% of the profit generated**, i.e. almost **three times more** than in *Switzerland*!
Thus, for a person who would like to create a company, the choice of *Switzerland* seems more favourable. Of course, funds will have to be found to be able to set up the company initially, but **taxation will be more favourable there in the long term**.

# **Conclusion**

On the basis of the selected indicators, we can first of all see that in most categories Switzerland is in first place or very close to first place in comparison with its three European neighbours, Germany, France and Italy. 

From an economic point of view, Switzerland is a very stable country (no negative development of its GDP over the last 10 years) with one of the highest GDP per capita in the world. This GDP per capita is explained by the high added value produced by Swiss companies, as evidenced by the pharmaceutical sector and the raw materials sector.
Switzerland also derives its stability from the fact that its trade balance has been positive since the 1970s, further demonstrating its ability to be self-sufficient but also and above all to export products and services at high prices.
Generally speaking, Switzerland is also competitive because it has been able to establish numerous trade partnerships with a large number of countries and its geographical position in the "centre" of Europe gives it many advantages in terms of transport costs (e.g. for its trade with Germany).
Finally, Switzerland has always been able to demonstrate excellent know-how in terms of products and services. Moreover, its advantageous tax system encourages many companies to set up their headquarters in Switzerland and thus benefit from the income generated.

From a social point of view, Switzerland excels here too. With one of the lowest unemployment rates in Europe, an excellent integration of young graduates and above all very high salaries which encourage many foreigners to come to Switzerland to work. 
The statistics in terms of crime also demonstrate Switzerland's ability to be a safe country for those who live there or those who would consider settling there. The risk of becoming poor or socially isolated is also below the European average.
All these good results logically lead Switzerland to obtain the best social progress index among the four countries compared, with a score of 89.9 out of 100, indicating a particularly high level of well-being for those living in Switzerland.
Although the social indicators are not directly correlated with the economic indicators, it is still important to note that the latter still have a minimal influence on social well-being. Indeed, a country with a very low GDP per capita and a high risk of poverty or social exclusion could not do as well in terms of the social progress index.

From a business point of view, Switzerland does not derive the majority of its income from watchmaking or luxury goods but from the sale of its services in the first instance. If we then look at its exports, the pharmaceutical industry is by far in the lead with the precious metals resale industry.
Germany and the USA are the preferred partners of the Swiss, both for imports and exports.
Returning to the service industry, it is important to note that the top five Swiss companies in terms of turnover are commodity trading companies. Some of them employ very few people but generate billions of CHF in sales. 
Nestlé remains the Swiss company with the most employees but is in 6th position in terms of turnover.
As far as the possibility of setting up a business in Switzerland is concerned, it is necessary to have a fairly large amount of start-up capital (unlike in France where this can be free of charge depending on the type of company chosen). Nevertheless, once the company has been created, taxation is much more advantageous in Switzerland. Thus, the entrepreneur will have more funds to be able to possibly carry out research and development, increase his production capacity or simply increase his salary.
Finally, Switzerland has a very good score on the corruption index, which means that it is a country where the environment is very favourable for entrepreneurship but also for living in general.