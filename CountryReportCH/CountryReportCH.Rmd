...
```{r}
CPIc <- CPIc[c(9,2,3,4,5,6,7,8,1,10,11,12,13,14,15,16,17)]
CPIc
```
...
library(dygraphs)
library(xts)
CPIcPlot <- xts(x = CPIc$value, order.by = CPIc$period)
```
...
```{r}
library(dplyr)
CPIcPlot %>% group_by(Country)
```


```{r}
library(dygraphs)
TestCPIc <- dygraph(CPIcPlot, main = "Country") %>% 
  dyRangeSelector(CPIc$period = c("1921-01-01", "2019-01-01"))
```
